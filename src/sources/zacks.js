...
const fetchData = async (logger, ticker, browser) => {
  ...
  try {
    ...
    const {
    ...
    } = await getSection(logger, "style scores", async () => {
      const styleUrl = `https://www.zacks.com/stock/research/${ticker}/stock-style-scores`
      const styleInterceptor = pageFetcher.addResponseInterceptor([styleUrl], true, {
        expectString: true,
      })
      await pageFetcher.setPage(styleUrl, { waitUntil: "domcontentloaded" })

      const styleHtml = await styleInterceptor.waitForResult()
      const fetcher = new JsDomFetcher()
      fetcher.setHTMLtoDOM(styleHtml)

      const [value, growth, momentum] = fetcher.getTextArrByX(`//thead//th[2]/span`)
      const [
        rank,
        vgm,
        cashPrice,
      ...
      const [
        ...
        pcfIndustry,
        peIndustry,
        priceToSalesIndustry,
        earningsYieldIndustry,
        debtEquityIndustry,
        ,
        ,
        ,
        histEpsGrowthIndustry,
        projEpsGrowthIndustry,
        currCashFlowGrowthIndustry,
        histCashFlowGrowthIndustry,
        currentRatioIndustry,
        debtCapitalIndustry,
        netMarginIndustry,
        roeIndustry,
        salesToAssetsIndustry,
        projSalesGrowthIndustry,
      ] = fetcher.getTextArrByX(`//tbody[2]/tr/td[3]`)

      return {
        zacksValue: value,
        zacksGrowth: growth,
        zacksMomentum: momentum,
        zacksRank: rank,
        zacksVGM: vgm,
        zacksCashPrice: cashPrice,
        zacksEVEbitda: evEbitda,
        zacksPB: pb,
        zacksPriceToSales: priceToSales,
        zacksEarningsYield: earningsYield,
        zacksDebtEquity: debtEquity,
        zacksCashFlowPerShare: cashFlowPerShare,
        zacksHistEpsGrowth: histEpsGrowth,
        zacksProjEpsGrowth: projEpsGrowth,
        zacksCurrCashFlowGrowth: currCashFlowGrowth,
        zacksHistCashFlowGrowth: histCashFlowGrowth,
        zacksCurrentRatio: currentRatio,
        zacksDebtCapital: debtCapital,
        zacksNetMargin: netMargin,
        zacksROE: roe,
        zacksSalesToAssets: salesToAssets,
        zacksProjSalesGrowth: projSalesGrowth,
        zacksEVEbitdaIndustry: evEbitdaIndustry,
        zacksPegTTMIndustry: pegTTMIndustry,
        zacksPBIndustry: pbIndustry,
        zacksPCFIndustry: pcfIndustry,
        zacksPEIndustry: peIndustry,
        zacksPriceToSalesIndustry: priceToSalesIndustry,
        zacksEarningsYieldIndustry: earningsYieldIndustry,
        zacksDebtEquityIndustry: debtEquityIndustry,
        zacksHistEpsGrowthIndustry: histEpsGrowthIndustry,
        zacksProjEpsGrowthIndustry: projEpsGrowthIndustry,
        zacksCurrCashFlowGrowthIndustry: currCashFlowGrowthIndustry,
        zacksHistCashFlowGrowthIndustry: histCashFlowGrowthIndustry,
        zacksCurrentRatioIndustry: currentRatioIndustry,
        zacksDebtCapitalIndustry: debtCapitalIndustry,
        zacksNetMarginIndustry: netMarginIndustry,
        zacksROEIndustry: roeIndustry,
        zacksSalesToAssetsIndustry: salesToAssetsIndustry,
        zacksProjSalesGrowthIndustry: projSalesGrowthIndustry,
      }
    }) || {}
